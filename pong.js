...
const ORIGINAL_PAD_H = PADDLE_H_BASE * PADDLE_HEIGHT_MULT;
const BALL_RADIUS = 7;
...
let state = {
  ...
  cpuY: 0,
  playerPrevY: 0,
  playerPadH: ORIGINAL_PAD_H,
  cpuPadH: ORIGINAL_PAD_H,
  balls: [],
  longestRally: 0,
  currentRallyTime: 0,
  fastestBall: 0,
...
function startMatch() {
  ...
  state.fastestBall = 0;
  state.playerPadH = ORIGINAL_PAD_H;
  state.cpuPadH = ORIGINAL_PAD_H;
  ...
  state.balls.forEach((ball) => {
    if (ball.vx < 0 && ball.x - BALL_RADIUS <= PADDLE_W) {
      if (ball.y > state.playerY && ball.y < state.playerY + state.playerPadH) {
        ...
        // Edge hit reduction
        if (Math.abs(hitPos) >= 0.7 && state.playerEdgeHits < 5) {
          state.playerPadH *= 19 / 20;
          state.playerEdgeHits++;
...
function commitLoss(role, type) {
  ...
  if (role === "player") {
    if (type === "life") {
      ...
      state.playerPadH = ORIGINAL_PAD_H * 0.5;
